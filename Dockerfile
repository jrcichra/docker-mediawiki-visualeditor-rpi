...
# Utilities
RUN apt-get update && \
    apt-get -y install apt-transport-https ca-certificates git curl --no-install-recommends --allow-unauthenticated && \
...
RUN apt-get update && apt-get install -y imagemagick libmagickwand-6.q16-dev --allow-unauthenticated --no-install-recommends && \
...
# Intl PHP extension
RUN apt-get update && apt-get install -y libicu-dev g++ --allow-unauthenticated --no-install-recommends && \
    docker-php-ext-install intl && \
    apt-get install -y --allow-unauthenticated --auto-remove libicu57 g++ && \
    rm -rf /var/lib/apt/lists/*
...
# Supervisor
RUN apt-get update && \
    apt-get install -y --allow-unauthenticated supervisor --no-install-recommends && \
    rm -r /var/lib/apt/lists/*
...
RUN curl -sL https://deb.nodesource.com/setup_4.x | bash - && \
    apt-get install -y nodejs --no-install-recommends --allow-unauthenticated
